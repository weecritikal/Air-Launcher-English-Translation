...
- **Minecraft starts in English.** A new instance was seeded with `lang:zh_cn`, so the game opened into a Chinese menu regardless of the launcher's language. New instances are seeded `en_us`; a language you have already chosen is left alone.
...
- **Modloader direct install works again.** The Forge version list was fetched from BMCLAPI on every request, ignoring the configured download source, while the installer jar came from whichever source that setting named. The two publish different sets of builds, so picking one the other did not carry failed with a bare `not found (404)` -- and changing the download source could not help, because the list never obeyed it. Listing and downloading now use the same source, the jar is tried across each available mirror, and a failure reports which hosts were tried.
