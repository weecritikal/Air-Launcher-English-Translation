...
- **An English interface.** Upstream's launcher is written in Mandarin; translating it is what this fork exists for. Every user-facing string is now English -- 1,313 hardcoded string literals across the Objective-C sources, 19 localization keys that were missing from `en.lproj`, and the in-source comments alongside them.
- **Other languages are left as they were.** The launcher still bundles the roughly fifty translations it inherited -- Japanese, Korean, Arabic, Hindi, Vietnamese and the rest -- unchanged. They have always been partial (Japanese covers 146 of 641 keys, for instance), and much of the newer interface is hardcoded English that does not follow the device language. Simplified and Traditional Chinese are the exception: those bundles came out along with the Mandarin interface they matched, rather than staying on as translations this fork has no way to keep accurate. If you want the launcher in Chinese, upstream [Amethyst-iOS-MyRemastered](https://github.com/herbrine8403/Amethyst-iOS-MyRemastered) is the better build -- it is maintained in that language by people who speak it.
...
- **Minecraft starts in English.** Upstream seeds a new instance with `lang:zh_cn`, which matches its own interface but meant the game opened into a Mandarin menu regardless of what language the launcher was in. New instances are seeded `en_us` instead. An instance still carrying the old default -- from an earlier build, or from an imported modpack's own `options.txt` -- is re-seeded once, after which whatever language the player picks is left alone.
...
- **Download sources are the official ones.** Upstream defaults every install to BMCLAPI, with MCIM for mods -- mirrors hosted in mainland China, and a sensible default for the users upstream is written for. They are a poor fit here: slower at this distance, and BMCLAPI carries a different set of Forge builds from the official maven, which is what surfaced as `not found (404)` when installing a modloader. Flux defaults to the official sources, and an install carrying the old setting is migrated on launch.
...
- **Official Download Sources** -- Game files come from Mojang's own servers, with Modrinth and CurseForge for mods, modpacks, shaders and resource packs.
- **English Interface** -- The launcher reads in English throughout. The partial translations inherited from upstream are still bundled for the strings they cover.
