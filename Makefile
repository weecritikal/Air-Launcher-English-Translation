...
METHOD_PACKAGE = \
	...
	rm -f $(OUTPUTDIR)/org.angelauramc.amethystremastered-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX; \
	rm -f $(OUTPUTDIR)/org.angelauramc.amethystremastered.slimmed-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX; \
...
payload: native dep_mg java jre assets
	...
	# 26.x LWJGL spvc 兼容：部分 LWJGL 版本仍查找默认名 libspirv-cross.dylib，
	# 而 Frameworks 下实际文件名为 libspirv-cross-c-shared.0.dylib。建立软链接双保险，
	# 与 Tools.java 中的 org.lwjgl.spvc.libname override 配合，避免 UnsatisfiedLinkError。
