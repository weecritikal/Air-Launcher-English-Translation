...
- (void)setupUI {
    self.modeSwitcher = [[UISegmentedControl alloc] initWithItems:@[@"Local resource packs", @"Search online (Modrinth)"]];
    ...
    self.importButton.accessibilityLabel = @"Import resource pack";
...
- (void)updateUIForCurrentMode {
    if (self.currentMode == ResourcePacksManagerModeLocal) {
        self.searchBar.placeholder = @"Search local resource packs...";
        self.emptyLabel.text = @"No resource packs found";
...
#pragma mark - Importing resource packs

- (void)importTapped {
    NSError *dirError = nil;
    NSString *dir = [[ResourcePackService sharedService] ensureResourcePacksFolderForProfile:self.profileName error:&dirError];
    if (!dir) {
        [self showSimpleAlertWithTitle:@"Cannot import" message:dirError.localizedDescription ?: @"Could not determine the resourcepacks folder"];
        return;
    }
    ...
    picker.title = @"Choose resource pack file";
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    ...
    NSString *dir = [[ResourcePackService sharedService] ensureResourcePacksFolderForProfile:self.profileName error:&dirError];
    if (!dir) {
        [self showSimpleAlertWithTitle:@"Import failed" message:dirError.localizedDescription ?: @"Could not determine the resourcepacks folder"];
        return;
    }
...
- (void)filterLocalItems {
    ...
    if (!self.emptyLabel.hidden) {
        self.emptyLabel.text = @"No local resource packs found";
    }
    [self.tableView reloadData];
}
